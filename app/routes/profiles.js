...
module.exports = (router) => {
  router.get("/", async (req, res) => {
    ...
    let response = formatResponse(profiles);
  ...
  router.get("/:id", async (req, res) => {
    try {
      var profile = await getProfile(req.params.id, req.query);
    } catch (error) {
      if (error.statusCode === 500) {
        console.error(error);
      }
    ...
    let response = formatResponse(profile);
    let status = response.status || 200;
    delete response.status;
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      var profile = await createProfile(req.body);
    } catch (error) {
      if (error.statusCode === 500) {
        console.error(error);
      }
  ...
  router.patch("/", async (req, res) => {
    try {
      var profile = await editProfile(req?.user?.id, req.body);
    } catch (error) {
      if (error.statusCode === 500) {
        console.error(error);
      }
  ...
  router.delete("/:id", async (req, res) => {
    try {
      var profile = await deleteProfile(req.params.id);
    } catch (error) {
      if (error.statusCode === 500) {
        console.error(error);
      }
